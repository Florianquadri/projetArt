...
const whatTypeEvent = computed(() => {
  ...
  if (props.typeEvent == "event") return "#FF8935";
  if (props.typeEvent == "course") return "#84F4BF";
  if (props.typeEvent == "devoir") return "#1D4EFF";
...
const whichColorFont = computed(() => {
  ...
  if (props.typeEvent == "event") return "white";
  if (props.typeEvent == "course") return "#262626";
  if (props.typeEvent == "devoir") return "white";
  ...
  <div class="period">
    ...
    <div class="infosJour">
      <div class="infosJourLigne">
        <span
          class="strong textCours"
          :style="{ fontSize: props.sizeFont + 'rem' }"
          >{{ props.cours }}</span
        >
        <span
          class="salle textCours"
          :style="{ fontSize: props.sizeFont + 'rem' }"
          >{{ salle }}</span
        >
      </div>
      <div class="infosJourLigne">
        <span
          v-if="user == 'teacher'"
          class="textCours strong classe"
          :style="{ fontSize: props.sizeFont + 'rem' }"
          >{{ props.classe }}</span
        >
        <span class="textCours" :style="{ fontSize: props.sizeFont + 'rem' }">{{
          debutCours
        }}</span>
        <span class="textCours" :style="{ fontSize: props.sizeFont + 'rem' }"
          >-{{ finCours }}</span
